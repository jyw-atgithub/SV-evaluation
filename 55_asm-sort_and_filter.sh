...
sed 's/ .       PASS/   30      PASS/g' |bgzip -@ ${nT} -c > ${SVs}/${name}.${prog}.filtered.vcf.gz
done


###########TO DO: try to remove homopolymers or multiple "N" in the mumco vcf###########
##We only want the DUPs from mumco
...
zcat ${SVs}/${name}.mumco.filtered.vcf.gz.ori|grep  "#" |bgzip -@ ${nT} -c > ${SVs}/${name}.mumco.filtered.vcf.gz.header
zcat ${SVs}/${name}.mumco.filtered.vcf.gz.ori|grep -v "#" |gawk -v OFS='\t' '{
    if ($5=="\<DUP\>") 
    {
...
#else {print $1,$2,$3,$4,$5,"20",$7,$8,$9,$10}